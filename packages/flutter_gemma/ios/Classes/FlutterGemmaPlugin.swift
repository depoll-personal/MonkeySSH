...
class PlatformServiceImpl : NSObject, PlatformService, FlutterStreamHandler {
    ...
    /// Keep inference lifecycle work serialized on iOS to avoid native
    /// MediaPipe runtime setup racing across arbitrary global-queue workers.
    private let inferenceQueue = DispatchQueue(
        label: "dev.flutterberlin.flutter_gemma.inference",
        qos: .userInitiated
    )
    ...
    private func runInferenceTask(_ task: @escaping @Sendable () -> Void) {
