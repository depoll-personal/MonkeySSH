...
class LiteRtLmClient {
  ...
  Future<String> createConversation({
    ...
    debugPrint(
        '[LiteRtLmClient] Conversation created: $_currentConversationId');
  ...
  Stream<String> chatWithImage(
    ...
    _assertInitialized();
    debugPrint(
        '[LiteRtLmClient] chatWithImage: text=${text.length} chars, image=${imageBytes.length} bytes');
  ...
  Future<void> cancelGeneration({String? conversationId}) async {
    ...
    try {
      ...
      if (response.hasError() && response.error.isNotEmpty) {
        debugPrint(
            '[LiteRtLmClient] Cancel generation error: ${response.error}');
      } else {
