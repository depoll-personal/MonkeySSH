...
class DesktopInferenceModelSession extends InferenceModelSession {
  ...
  Future<void> addQueryChunk(Message message) async {
    _assertNotClosed();
    debugPrint(
        '[DesktopSession] addQueryChunk: hasAudio=${message.hasAudio}, audioBytes=${message.audioBytes?.length}, supportAudio=$supportAudio');
  ...
  Stream<String> getResponseAsync() async* {
    ...
    debugPrint(
        '[DesktopSession] getResponseAsync: audio=${audio?.length}, image=${image?.length}');

    if (audio != null) {
      debugPrint(
          '[DesktopSession] Calling chatWithAudio: audio=${audio.length} bytes');
      yield* grpcClient.chatWithAudio(text, audio);
    } else if (image != null) {
      debugPrint(
          '[DesktopSession] Calling chatWithImage: image=${image.length} bytes');
