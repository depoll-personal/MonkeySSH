...
class ServerProcessManager {
  ...
  static ServerProcessManager get instance =>
      _instance ??= ServerProcessManager._();
  ...
  Future<void> start({int? port, int? maxHeapMb}) async {
    if (_serverProcess != null) {
      debugPrint(
          '[ServerProcessManager] Server already running on port $_currentPort');
      return;
    }
    ...
    try {
      ...
      _serverProcess!.stderr.transform(utf8.decoder).listen((line) {
        ...
        if (line.contains('Exception') && !startupCompleter.isCompleted) {
          startupCompleter
              .completeError(Exception('Server failed to start: $line'));
        }
      });
      ...
      _serverProcess!.exitCode.then((code) {
        debugPrint(
            '[ServerProcessManager] Server process exited with code $code');
        _serverProcess = null;
        if (!startupCompleter.isCompleted) {
          startupCompleter
              .completeError(Exception('Server process exited unexpectedly'));
        }
      });
      ...
      debugPrint(
          '[ServerProcessManager] Server started successfully on port $_currentPort');
  ...
  Future<void> stop() async {
    ...
    try {
      await _serverProcess!.exitCode.timeout(
        ...
        onTimeout: () {
          debugPrint(
              '[ServerProcessManager] Graceful shutdown timed out, force killing');
  ...
  Future<String?> _getBundledJrePath() async {
    ...
    if (Platform.isMacOS) {
      ...
      jrePath =
          path.join(executableDir, '..', 'Resources', 'jre', 'bin', javaExe);
  ...
  Future<String> _getJarPath() async {
    ...
    if (Platform.isMacOS) {
      ...
      jarPath =
          path.join(executableDir, '..', 'Resources', 'litertlm-server.jar');
    } else if (Platform.isWindows) {
