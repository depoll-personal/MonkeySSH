...
abstract class SSHKexUtils {
  ...
  static Uint8List computeExchangeHash({
    ...
    required Uint8List sharedSecretEncoding,
  }) {
    ...
    writer.writeBytes(sharedSecretEncoding);
  ...
  static Uint8List encodeMpintSharedSecret(BigInt sharedSecret) {
    final writer = SSHMessageWriter();
    writer.writeMpint(sharedSecret);
    return writer.takeBytes();
  }

  static Uint8List encodeBinarySharedSecret(Uint8List sharedSecret) {
    final writer = SSHMessageWriter();
    writer.writeString(sharedSecret);
    return writer.takeBytes();
  }

  static Uint8List hashSharedSecret({
    required Digest digest,
    required Iterable<Uint8List> parts,
  }) {
    final builder = BytesBuilder(copy: false);
    for (final part in parts) {
      builder.add(part);
    }
    final input = builder.takeBytes();
