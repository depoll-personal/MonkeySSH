...
class SSHTransport {
  ...
  /// cipher IV, cipher key and MAC key.
  Uint8List? _sharedSecretEncoding;
  ...
  Uint8List _deriveKey(SSHDeriveKeyType keyType, int keySize) {
    return SSHKexUtils.deriveKey(
      ...
      sharedSecretEncoding: _sharedSecretEncoding!,
  ...
  void _handleMessageKexReply(Uint8List payload) {
    ...
    late Uint8List sharedSecretEncoding;
    ...
    if (kex is SSHKexDH) {
      ...
      sharedSecretEncoding = SSHKexUtils.encodeMpintSharedSecret(
        kex.computeSecret(message.f),
      );
    } else if (kex is SSHKexECDH) {
      ...
      sharedSecretEncoding = kex.computeSecretEncoding(message.ecdhPublicKey);
    } else {
    ...
    _sharedSecretEncoding = sharedSecretEncoding;
